class Task < ApplicationRecord
  ...
  scope :by_limit, -> { order(limit: :asc) }
  scope :by_priority, -> { order(priority: :asc) }

  # search_with
  # scope :current_user, -> (current_user) { where user_id: current_user.id}
  scope :title, -> (search_title) { where('title LIKE ?', "%#{search_title}%") if search_title.present? }
  scope :status, -> (search_status) { where status: search_status if search_status.present? }
  scope :label, -> (search_labels) { where(labels: { id: search_labels }) if search_labels.present? }
