...
before do
  ...
  task = FactoryBot.create(:task, user: @user) #ファクトリーボットはアソシエーションしてると毎回ユーザーまで作ってしまいます。メアドのユニークネスエラーを誘発します。
  # task2 = FactoryBot.create(:task, user: user1)
  # task3 = FactoryBot.create(:task, user: user)
  ...
  describe 'タスク一覧画面' do
    ...
    context '複数のタスクを作成した場合' do
      it 'タスクが作成日時の降順に並んでいること' do
        new_task = FactoryBot.create(:task, title: 'new_task', user: @user)
        visit tasks_path
      ...
      it 'タスクが終了期限の早い順に並んでいること' do
        new_task = FactoryBot.create(:task, title: 'new_task', limit: '2020-03-01', user: @user)
        new_task = FactoryBot.create(:task, title: 'old_task', limit: '2020-01-01', user: @user)
      ...
      it 'タスクが優先度の高い順に並んでいること' do
        new_task = FactoryBot.create(:task, title: 'important_task', priority: 0, user: @user)
        new_task = FactoryBot.create(:task, title: 'not_important_task', priority: 2, user: @user)
